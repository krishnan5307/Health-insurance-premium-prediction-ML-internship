...
class Pipeline(Thread):
    ...
    def start_data_ingestion(self) -> DataIngestionArtifact:
        try:
            data_ingestion = DataIngestion(data_ingestion_config=self.config.get_data_ingestion_config())   ## calling componennt
    ...
    def start_data_validation(self, data_ingestion_artifact: DataIngestionArtifact) \
        ...
        try:
            data_validation = DataValidation(data_validation_config=self.config.get_data_validation_config(),    ## calling componennt
    ...
    def run_pipeline(self):    
        try:
            ...
            experiment_id = str(uuid.uuid4())  ## to generate unique id for each action un UI (eg: training started, completed etc)

            Pipeline.experiment = Experiment(experiment_id=experiment_id,                             ## starting the expeiment
            ...
            Pipeline.experiment = Experiment(experiment_id=Pipeline.experiment.experiment_id,    ## stopping/completion of the experimnt
                                             initialization_timestamp=self.config.time_stamp,
                                             artifact_time_stamp=self.config.time_stamp,
